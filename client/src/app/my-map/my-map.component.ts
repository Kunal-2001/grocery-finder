...
export class MyMapComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
      this.fetchlocationService.getcurrentLocation().subscribe(locat => this.current_location = locat);
      console.log(this.current_location)

      //const initialState = { lng: position.coords.longitude, lat: position.coords.latitude, zoom: 15 };
      this.lefletMap = L.map(this.mapContainer.nativeElement).setView([this.current_location.longitude, this.current_location.latitude], 15);
      ...
      L.marker([this.current_location.longitude, this.current_location.latitude]).addTo(this.lefletMap)
